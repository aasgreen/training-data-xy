 ...
 !          the force (torque) on the spin at site i,j
        ...
        select case (torqueScheme)
            ...
            case (1)
                ...
                torque = kappa*(1-alpha*energy/kappa)*(sin(x(2)-x(1))+ &
    ...
    subroutine findDefects(dgrid,grid,n)

        integer, intent(in) :: n
        real(8), dimension(n,n), intent(in) :: grid
        real(8), dimension(n,n), intent(out) :: dgrid
...
!
 ...
 !          the force (torqueLandau) on the spin at site i,j
        ...
        y =(/ grid(i,modulo(j-2,n)+1),grid(i,j),grid(i,modulo(j,n)+1)/)
        energy = kinetic(i,j,grid,n,kappa)
        torqueLandau = kappa*(1-alpha*energy/kappa)*(sin(x(2)-x(1))+ &
            sin(x(2)-x(3))+sin(y(2)-y(1))+sin(y(2)-y(3)))+4*mu*cos( (grid(i,j)-fieldphi))* &
            sin((grid(i,j)-fieldphi) )
    ...
    function kinetic(i,j,grid,n,kappa)
        ...
        x =(/ grid(modulo(i-2,n)+1,j),grid(i,j),grid(modulo(i,n)+1,j)/)
        y =(/ grid(i,modulo(j-2,n)+1),grid(i,j),grid(i,modulo(j,n)+1)/)
        kinetic = -kappa*(-4+(cos(x(2)-x(1))+cos(x(2)-x(3))+cos(y(2)-y(1))+cos(y(2)-y(3)))) !shift so energy is bounded
        !(0,kappa).
   ...
   function anchor(ii,jj,ir,dr,n)
    ...
    subroutine update(state,newstate,hgrid,lnoise,kappa, mu, n,alpha,deltime)
        real(8) :: theta, x,lnoise,fluce,force,fieldphi,mutemp, kappa, mu
        integer :: n,ii,jj,i,j
        real(8), intent(in), optional :: alpha
        real(8), dimension(n,n) :: state,newstate,hgrid

        intent(in) :: state, n, lnoise, kappa, mu, deltime
